...
: AForm("PresForm", 25, 5), _target("?")
{
	if (DEBUG)
		std::cout << GREEN "[Default Constructor]\t[Pres]\t" RESET << *this << RESET << std::endl;
}
...
{
	if (DEBUG)
		std::cout << GREEN "[Copy Constructor]\t[Pres]\t" RESET << *this << RESET << std::endl;
}

PresidentialPardonForm::PresidentialPardonForm(std::string target) 
: AForm("PresForm", 25, 5), _target(target)
{
	if (DEBUG)
		std::cout << GREEN "[String Constructor]\t[Pres]\t" RESET << *this << RESET << std::endl;
}
...
{
	if (DEBUG)
		std::cout << RED "[Destructor]\t\t[Pres]\t" RESET << *this << RESET << std::endl;
}
...
{
    if (DEBUG)
        std::cout << CYAN "[Assignment Operator]\t[Pres]\t" RESET << *this;
	// AForm
