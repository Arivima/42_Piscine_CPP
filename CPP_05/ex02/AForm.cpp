...
void			AForm::beSigned(Bureaucrat const &b)
{
	...
	{
		std::cerr << BOLDRED << b.getName() << " couldn't sign " << this->getName() << " because: " << err << RESET << std::endl;
	}
...
void			AForm::execute(Bureaucrat const & executor) const
{
	...
	try
	{
		if (executor.getGrade() <= this->getGradeToExec())
		{
			...
			{
				throw "Form hasn't been signed yet ";
			}
		}
		else
		{
	...
	{
		std::cerr << BOLDRED << executor.getName() << " couldn't execute " << this->getName() << " because: " << err << RESET << std::endl;
	}
	return;
}
